...
自从发现用 `window.addEventListener('click', handler)` 可以很方便地用来实现收起 React 中的 Dropdown 后，我就不亦乐乎的到处用起来了。为了避免写无数遍的 `window.addEventLister('click', handler)`，我封装了一个 [NativeClickListener](../src/NativeClickListener.js) 的 Component，代码没几行，如下所示：
...
使用 (详见 [Dropdown5.js](../src/Dropdown5.js))：
...
于是实现 [NativeClickListener2](../src/NativeClickListener2.js)：
...
使用 (详见 [Dropdown6.js](../src/Dropdown6.js))：
...
![](../art/native-click-listener-2.gif)
